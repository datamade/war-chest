            ...
            for report in reports:
                ...
                if raw_period:
                    ...
                    if len(period) > 1:
                        ...
                        report_data['period_from'] = date(int(f_year), int(f_month), int(f_day))
                        report_data['period_to'] = date(int(t_year), int(t_month), int(t_day))
                    else:
                        year = raw_period.split(' ')[0]
                        try:
                            report_data['period_from'] = date(int(year), 1, 1)
                        except ValueError:
                            pass
                ...
                if detailed is not None:
                    ...
                    if report_id:
                        ...
                        if funds_start:
                            report_data['funds_start'] = float(funds_start[0].text.replace(',', '').replace('$', ''))
                        funds_end = report_detail.xpath("//span[@id='%s']" % self.detail_funds_end)
                        if funds_end:
                            report_data['funds_end'] = float(funds_end[0].text.replace(',', '').replace('$', ''))
                        expenditures = report_detail.xpath("//span[@id='%s']" % self.detail_expend)
                        if expenditures:
                            report_data['expenditures'] = float(expenditures[0].text.replace(',', '').replace('$', ''))
                        receipts = report_detail.xpath("//span[@id='%s']" % self.detail_receipts)
                        if receipts:
                            report_data['receipts'] = float(receipts[0].text.replace(',', '').replace('$', ''))
                else:
